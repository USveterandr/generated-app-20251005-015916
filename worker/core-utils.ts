...
export type IS<T> = T extends new (env: Env, id: string) => IndexedEntity<infer S> ? S : never;
export type HS<TCtor> = TCtor & { indexName: string; keyOf(state: IS<TCtor>): string; seedData?: ReadonlyArray<IS<TCtor>> };
export type CtorAny = new (env: Env, id: string) => IndexedEntity<{ id: string }>;
